...
def append_inspector_row_to_excel(form: Dict[str, Any]) -> bool:
    ...
    # Ищем последнюю заполненную строку по столбцу B (дата выезда)
    last_data_row = 1
    ...
    # Сбрасываем кэш, чтобы новые данные подхватились в ОНзС/Замечаниях
    REMARKS_CACHE[\"mtime\"] = None
    REMARKS_CACHE[\"df\"] = None

    log.info(
        \"Инспектор добавил выезд (строка %s) в лист %s\",
        new_row,
        INSPECTOR_SHEET_NAME,
    )
    return True
...
# --------- 👷 Инспектор ---------
async def handle_menu_inspector(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    kb = InlineKeyboardMarkup(
        [
...
async def handle_inspector_step(
    ...
    # Шаг 9: вид проверки -> сохраняем
    if step == \"check_type\":
        form[\"check_type\"] = text
        # Пытаемся записать в Excel
