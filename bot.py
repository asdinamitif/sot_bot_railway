...
def download_file_from_yandex(public_url: str) -> bytes:
    """
    Скачивает реальный файл по публичной ссылке Яндекс.Диска.
    Работает со всеми ссылками вида:
      https://disk.yandex.ru/i/XXXX
      https://disk.yandex.by/i/XXXX
    """
    try:
        log.info(f"Пробую получить прямой href Яндекс.Диска для URL: {public_url}")
        ...
        href = data["href"]
        log.info(f"Получен прямой href: {href}")

        file_resp = requests.get(href, timeout=60)
        file_resp.raise_for_status()

        log.info(f"Файл скачан с Яндекс.Диска, размер: {len(file_resp.content)} байт")
        return file_resp.content

    except Exception as e:
        log.error(f"Ошибка скачивания с Яндекс.Диска: {e}")
        raise
