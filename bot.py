...
# ----------------- ЗАГРУЗКА ФАЙЛОВ С ЯНДЕКС.ДИСКА / GOOGLE / ПРЯМЫХ ССЫЛОК -----------------
def download_file_from_yandex(public_url: str) -> bytes:
    """
    Скачивает файл по публичной ссылке.

    Поддерживает:
      • Яндекс.Диск: https://disk.yandex.ru/d/...   или https://disk.yandex.by/d/...
      • Google Sheets: https://docs.google.com/spreadsheets/d/...   (экспорт в .xlsx)
      • Прямые ссылки на файл (любой URL) — просто скачиваем как есть.
    """
    try:
        # -------- Google Sheets --------
